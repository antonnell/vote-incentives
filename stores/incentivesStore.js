...
class Store {
        ...
        for (let j = 0; j < flatBriberies.length; j++) {
          ...
          for (let i = 0; i < bribery.length; i++) {
            ...
            rewards.push({
              ...
              claimable: BigNumber(bribe.claimable)
                .div(10 ** bribe.rewardToken.decimals)
                .toFixed(bribe.rewardToken.decimals),
              canClaim: bribe.canClaim,
              hasClaimed: bribe.hasClaimed,
              gauge: bribe.gauge,
              tokensForBribe: BigNumber(bribe.tokensForBribe)
                .div(10 ** bribe.rewardToken.decimals)
                .toFixed(bribe.rewardToken.decimals),
  ...
  _getTokenInfo = async (web3, tokenAddress, getBalance) => {
    try {
      ...
      const [symbol, decimals] = await Promise.all([token.methods.symbol().call(), token.methods.decimals().call()]);
      ...
      votes.map(async (vote) => {
        ...
        const rewards = await Promise.all(
          vote.rewardsPerVote.map(async (rewardTokenAddress) => {
            ...
            return {
              estimateBribe: BigNumber(estimateBribe)
                .div(10 ** rewardToken.decimals)
                .toFixed(rewardToken.decimals),
  ...
  _getBribery = async (web3, account, gauges, rewardTokens, rewardTokenAddress) => {
    ...
    const [gaugesPerRewardV2] = await Promise.all([briberyV2.methods.gauges_per_reward(rewardTokenAddress).call()]);
    ...
    if (gaugesPerRewardV2.length > 0) {
      briberyResultsPromisesV2 = gaugesPerRewardV2.map(async (gauge) => {
        ...
        return {
          ...
          gauge: gauges.filter((g) => {
            return g.gaugeAddress.toLowerCase() === gauge.toLowerCase();
          })[0],
          rewardToken: rewardTokens.filter((r) => {
  ...
  _getCurrentGaugeVotes = async (web3, account, gauges) => {
    ...
    for (let i = 0; i < gauges.length; i++) {
      ...
      gauges[i].votes.userVoteSlopeAmount = BigNumber(userVoteSlopes[i].slope)
        .div(10 ** 10)
        .toFixed(10);
  ...
  addReward = async (payload) => {
    ...
    let sendAmount = BigNumber(rewardAmount)
      .times(10 ** rewardToken.decimals)
      .toFixed(0);
  ...
  _callAddVoteReward = async (web3, account, voteIndex, rewardToken, rewardAmount, callback) => {
    ...
    this._callContractWait(
      ...
      bribery,
      'add_reward_amount',
      [voteIndex, rewardToken, rewardAmount],
      account,
      gasPrice,
      GET_INCENTIVES_BALANCES,
      {},
